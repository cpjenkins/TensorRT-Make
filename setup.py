...
def make_libtorchtrt_pre_cxx11_abi(
    develop=True, use_dist_dir=True, cxx11_abi=False, rt_only=False
):
    cmd = ["make", "-f", "torch_tensorrt.mk", "-j8"]
...
def build_libtorchtrt_pre_cxx11_abi(
    ...
    if MAKEFILE:
        return make_libtorchtrt_pre_cxx11_abi(develop=develop, use_dist_dir=use_dist_dir, cxx11_abi=cxx11_abi, rt_only=rt_only)
    cmd = [BAZEL_EXE, "build"]
    if rt_only:
...
def copy_libtorchtrt(multilinux=False, rt_only=False):
    print("copy_libtorch", dir_path, 'multilinux', multilinux)
...
class DevelopCommand(develop):
    ...
    def run(self):
        ...
        if not PY_ONLY:
            ...
            # )
            make_libtorchtrt_pre_cxx11_abi(
                develop=True, cxx11_abi=CXX11_ABI, rt_only=NO_TS
            )
